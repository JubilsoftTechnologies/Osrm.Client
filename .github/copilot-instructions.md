...
- For local NuGet release preparation, use `./scripts/publish-package.sh <version> --skip-push` or `pwsh ./scripts/publish-package.ps1 -Version <version> -SkipPush`; both scripts also support live push with `NUGET_API_KEY`.
...
- Response models live under `Models/Responses` plus shared domain types in `Models/`. Geometry-aware models (`Route`, `RouteStep`, and related wrappers) decode `polyline`, `polyline6`, and `geojson` payloads on demand through `OsrmGeometryParser`.
...
- Shared request validation lives in `BaseRequest.Validate()`. Derived requests with stricter coordinate-count rules should still run the base validation so common options like `approaches`, `exclude`, and `snapping` stay enforced consistently.
...
- Packaging is explicit now: use `dotnet pack` locally, the local publish scripts for release preparation or direct push, `ci.yml` for restore/build/test/pack validation, and `publish-package.yml` for manual GitHub Actions-based NuGet publishing with `NUGET_API_KEY`.
